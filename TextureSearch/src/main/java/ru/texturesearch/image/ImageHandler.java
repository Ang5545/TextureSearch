...
public class ImageHandler {
	...
	public boolean isUniformLbp(String bits) {
		char c = 0;
	...
	public int getLbp(String val) {
		int result = 0;
		if (val.charAt(0) == '1') result += 1;
		if (val.charAt(1) == '1') result += 2;
		if (val.charAt(2) == '1') result += 4;
		if (val.charAt(3) == '1') result += 8;
		if (val.charAt(4) == '1') result += 16;
		if (val.charAt(5) == '1') result += 32;
		if (val.charAt(6) == '1') result += 64;
		if (val.charAt(7) == '1') result += 128;
		return result;
	}
...
// --------------------------------------------------------------------------------------------------------
		    	...
		    	int index = y * ws + x * ch;  
		        int val = 	getValFromByteBuffer(x, y, bb_img, ws, ch);
		        int val_0 = getLBPval(val, getValFromByteBuffer(x, y+1, bb_img, ws, ch));
		        int val_1 = getLBPval(val, getValFromByteBuffer(x+1, y+1, bb_img, ws, ch));
		        int val_2 = getLBPval(val, getValFromByteBuffer(x+1, y, bb_img, ws, ch));
		        int val_3 = getLBPval(val, getValFromByteBuffer(x+1, y-1, bb_img, ws, ch));
		        int val_4 = getLBPval(val, getValFromByteBuffer(x, y-1, bb_img, ws, ch));
		        int val_5 = getLBPval(val, getValFromByteBuffer(x-1, y-1, bb_img, ws, ch));;
		        int val_6 = getLBPval(val, getValFromByteBuffer(x-1, y, bb_img, ws, ch));
		        int val_7 = getLBPval(val, getValFromByteBuffer(x- 1, y+1, bb_img, ws, ch));
		        ...
		        if (isUniformLbp(bits)) {
		        	int resultVal = getLbp(bits);
		        	bb_result.put(index, (byte) resultVal);	   
		        	lbpList.add(resultVal);
		        }
